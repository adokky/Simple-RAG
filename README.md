...
## Использование стандартного RAG pipeline из langchain4j

Реализация есть `dev` бранче. Результаты хуже. 

Пока не ясно как можно реализовать свою стратегию эмбеддинга, используя в качестве входа `TextSegment`. В langchain4j хардкодом задается векторизация текстового содержимого `text()`. 

Возможно проблему можно решить иначе - через кастомизацию augmentor.
